    ...
    if (urlProjectId !== projectId) {
      throw new Error(
        `Supabase config mismatch: SUPABASE_URL host '${urlProjectId}' does not match VITE_SUPABASE_PROJECT_ID '${projectId}'. ` +
          "Update both values so the server and Supabase project reference the same project ref."
      );
    }
  } catch (error) {
...
export const requireSupabaseAuth = createMiddleware({ type: "function" }).server(
  async ({ next }) => {
    ...
    validateSupabaseConfig(SUPABASE_URL, process.env.VITE_SUPABASE_PROJECT_ID);
